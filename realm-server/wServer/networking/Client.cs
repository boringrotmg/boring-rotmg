    ...
    public partial class Client
    {
        ...
        internal void ProcessPacket(Packet pkt)
        {
            using (TimedLock.Lock(DcLock))
            {
                ...
                try
                {
                    Log.Trace("Handling packet '{0}'...", pkt.GetPacketId);

                    IPacketHandler handler;
                    if (!PacketHandlers.Handlers.TryGetValue(pkt.GetPacketId, out handler))
                        Log.Warn("Unhandled packet '{0}'.", pkt.GetPacketId);
                    else
